...
for (var in na.omit(vars[vars$Test == "Wilcoxon", "Variable"])){
  print(var)
  #group_by(dcs, Data.Collection.Number) %>% summarise(
   # median = median(var, na.rm = TRUE),
    #IQR = IQR(var, na.rm = TRUE))
}    
dcs %>%
  group_by(Sex) %>%
  get_summary_stats(Isolation.Response..Barking., type = "median_iqr")

r = group_by(dcs, Sex) %>% summarise(
    mean = mean(Isolation.Response..Barking., na.rm = TRUE),
    median = median(Isolation.Response..Barking., na.rm = TRUE),
    IQR = IQR(Isolation.Response..Barking., na.rm = TRUE))
