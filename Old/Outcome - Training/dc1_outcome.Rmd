...
dc1 <- read.csv('0_data/2_prepare/2022-07-22_Ethogram-Trainers-DC1.csv', 
          # imports character columns as factors
...
h
suppressMessages(ggsave("3_analysis/dc1/duration-histogram.png",h))
...
write.csv(dc_lr, "3_analysis/dc1/univariate-logistic regression.csv")
write.csv(dc_lr %>% filter(p_value_1 < 0.2) %>% select(estimate_1, p_value_1), "3_analysis/dc1/significant-variables.csv")
```
## Feature Selection
...
<!--smaller p value was kept and the other one was dropped. -->
```{r, echo = FALSE}
# select original ethogram items and scores based on the p values
dc_ord_feat <- dc_ord_res %>% filter(p_value_1 < 0.2)  %>% rownames()
dc_cat_feat <- dc_cat_lr_res_2 %>% filter(p_value_1 < 0.2)  %>% rownames()
dc_sign <- dc %>% select(all_of(dc_ord_feat), all_of(dc_cat_feat), Outcome)

# original variables only
# dc_sign <- dc_sign %>% select(-c(matches("^S[.]"))) 
...
#             "S.Crate.Stimulus_mean", 

#             # "Crate.Behaviours..Actively.Seeking.Attention.",
#             "S.Crate.Handler_mean",# "S.Crate.Handler_prod"

#             # "Petting.Confidence.During", "Petting.Responsiveness.After"

              # "S.Sociability",
#             "S.Petting.Stimulus.During_mean",
...
feat_drop <- 
        c( "Body.check.Table",
            ...
            "S.Crate.Handler_mean", # should always be dropped

            # "S.Sociability_mean",

            "Petting.Confidence.During", "Petting.Responsiveness.After", #both are included in sociability
            "S.Petting.Stimulus.During_mean",
            "S.Petting.Stimulus.During_prod",  # should always be dropped
            "S.Petting.Engagement.During_mean"
            )
...
# table(dc_row_nas)
# reduced dataset
dc_col_nas <- unlist(lapply(dc_sign, function (x) sum(is.na(x))))
print(dc_col_nas)
...
```{r, echo = FALSE}
#categorical
cat_feat <- dc_sign %>% select(where(is.factor), -Outcome) %>% colnames()
...
plot2 <- ggplot(data = dc_sign) +
        geom_mosaic(aes(x=product(Tea.Towel.First.Response..Indifferent., Outcome), fill = Outcome), alpha = 0.5) +
...
suppressMessages(ggsave(paste("3_analysis/dc1/", 
...
A total observations total of 13 observations were removed from the original 99 observations due to missing data. 
The multivariate logistic regression model was fit to 86 observations.
VIF values for DC data indicate a low degree of multicollinearity between the variables selected based on the univariate logistic regression model. 
...
print("PCA dataset after keeping 95% variance")
dc_prep <- impute_pca(dc_sign)
dc_pca  <- bake(dc_prep, dc_sign)
str(dc_pca)

print("Modelling using Recursive Feature Elimination (RFE) to optimize for AUC-ROC using Leave One Out Cross-Validation (LOOCV)")
...
plot
suppressMessages(ggsave(paste("3_analysis/dc1/rfe-roc.png"), plot))
```


NEW: Model fitness with deviance analysis. Performance metrics ROC and PR curves. 
```{r, echo = FALSE}
# model fitness
dev = model$null.deviance - model$deviance
deg = model$df.null - model$df.residual
...
df_perf <- model_rfe$pred %>% 
            filter(Variables == 7) %>%
            select(Variables, rowIndex, Fail, pred, obs) %>%
...
roc_curve <- roc.curve(scores.class0 = df_perf$Probability,
             ...
             curve=TRUE)
roc_curve
png('results/dc1-new/roc-curve.png')
...
pr_curve <- pr.curve(scores.class0 = df_perf$Probability,
             ...
             curve=TRUE)
pr_curve
png('results/dc1-new/pr-curve.png')
plot(pr_curve)
dev.off()
```
...
print("Probability predicted by the LR using DC1 ethogram data vs true Outcome")
plot <- ggplot(perf, aes(x = Estimate, y = Probability, color = Outcome )) +
    ...
    geom_point()
plot
suppressMessages(ggsave(paste("3_analysis/dc1/logit-outcome.png"), plot))


print("Probability predicted by the LR using DC1 ethogram data vs true Status")
...
plot <- ggplot(perf, aes(x = Estimate, y = Probability, color = Status )) +
    ...
    geom_point()
plot
suppressMessages(ggsave(paste("3_analysis/dc1/logit-status.png"), plot))
...
dc_pca_var$component <- mapvalues(dc_pca_var$component, from = 1:9, 
...
dc_pca_var$Included = factor(c("Yes", "No", "Yes", "No", "Yes", "No", "Yes", "Yes", "No"))
plot <- ggplot(dc_pca_var, aes(x = component, y = percent.variance, fill = Included)) +
    geom_bar(stat='identity', position = 'dodge', width = 0.8)+
    xlab("Principal components") +
    ylab("Explained Variance (%)") 
plot
suppressMessages(ggsave(paste("3_analysis/dc1/pc-explained-variance.png"), plot))
...
# suppressMessages(ggsave(paste("3_analysis/dc1/pc-cumulative-variance.png"), plot))
# plot
...
# figuring out the righest loading items for the PC1K
...
suppressMessages(ggsave(paste("3_analysis/dc1/pc-loadings.png"), plot))
plot
