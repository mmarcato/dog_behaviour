...
tab_results <- function(df, name, caption=NULL) {
    ...
    path = paste("3_analysis/dc2-test/", name, ".csv", sep = "")
...
fig_results <- function(plot, name, caption = NULL, label = NULL){
    ...
    path = paste("3_analysis/dc2-test/", name, ".png", sep = "")
...
tab_results(table(dc$Kong.Interaction.Lateralisation), name = 'descriptive-kong-lateralisation')
```
...
```{r, echo = FALSE}
# visualise features based on univariate analysis the p values: 32 features < 0.25
dc_lr %>% filter(p_value_1 > 0 & p_value_1 < 0.25)  %>% select(p_value_1) %>% arrange(desc(p_value_1))
...
feat_drop_items <- c( 
            ...
            # removing because of "S.Kong.Response_prod",
            "Kong.Interaction.Response.to.stimulus", # p-value smaller
            "Kong.Interaction.Back",
            "Kong.Return.Handler", # p-value smaller
            ...
            # IMPORTANT: when removing the scores below, comment out original features with p-value < than threshold
            ...
            # removing because of "S.Distractions.First.Response_mean",
            # "Distractions.First.Response..Car.", # p-value = 0.02
            ...
            # removing because of "S.Sensitivity_mean",
            # "Body.check.Table", # p-value = 0.07
            
            # removing because of "S.Crate.Handler_mean",
            "Crate.Behaviours..Actively.Seeking.Attention.", # p-value = 0.14
            "Crate.Behaviours..Whining.", # p = 0.21
            ...
            # removing because of "S.Isolation.Handler_mean",
            # "Isolation.Response..Time.oriented." # p-value = 0.05
            )
```

```{r, echo = FALSE}
# remove variables with p>0.25
data_1 <- dc %>% 
    select(all_of(dc_lr %>% filter(p_value_1 > 0 & p_value_1 < 0.25) %>% rownames()),
             Kong.Interaction.Lateralisation, Outcome)
data_1 %>% colnames()

# remove associated original ethogram items and scores
data_1 <- data_1 %>% select(-all_of(feat_drop_items))
data_1 %>% colnames()
