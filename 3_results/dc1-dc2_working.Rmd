...
tab_results <- function(df, name, caption=NULL) {
    ...
    #     label = paste("T-dc1-dc2-", name, sep = "")),
...
dc1 <- dc1 %>% filter(Name %in% dc2$Name) 
...
dc_cat <- dc1 %>% select(all_of(cat), -Isolation.Urinating, -Crate.Urinating, Breed, Sex, Working)
...
# p<0.15, scores (pvalue)
data_1 <- dc1 %>% select(Working, S.Familiarisation.Handler_mean, Walking.Initiative, S.Distractions.Pull_mean, S.Walking.Distractions.Pull_prod,
               S.Distractions.Difference_mean, S.Crate.Stimulus_mean, S.Crate.Handler_mean, S.Isolation.Handler_prod)
...
data_reduced <- data_1 %>% select(-S.Isolation.Handler_prod, -S.Walking.Distractions.Pull_prod)
```
...
png('3_results/dc1-dc2-working/roc-curve.png')
...
pr_curve <- pr.curve(scores.class0 = df_perf$Probability,
             ...
             curve=TRUE)
plot(pr_curve)
png('3_results/dc1-dc2-working/pr-curve.png')
