...
data <- read.csv('20-06-23_Ethogram.csv', stringsAsFactors=FALSE)
...
dc1 = dc1 %>% filter( Status != 'in Training' )
...
dc2 = dc2 %>% filter( Status != 'in Training' )
# printing out information
...
dogs_dcs = dog_dc[dog_dc$x ==3, 'Category']
print(dim(dcs))
dcs = data %>% filter( Name %in% dogs_dcs, Status != 'in Training' )
print(dim(dcs))
# removing litter with only one dog
...
ordinal_between_groups <- function(df){
  ...
  for (variable in ordinal){
      print(variable)
      r = kruskal.test(df[, variable], df$Status)
...
# saves results for ALL ORDINAL-scored behaviours between Status
...
cat('Ordinal - DC 1 - Difference between outcome')
print(dc1_ordinal_sign))
cat('Ordinal - DC 2 - Difference between outcome')
...
categorical_between_groups <- function(df){
  ...
  for (variable in categorical){
    ...
    if (length(unique(df[,variable]))>1){
      r = chisq.test(df[, variable], df$Status, simulate.p.value=TRUE)
